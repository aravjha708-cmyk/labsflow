/**
 * Google Sign-In Overlay Module (1:1 Identical match to Flow Labs/ext/flow_login_overlay.js)
 */
function getLoginOverlayScript(gateEnabled) {
  return `
    <script id="__flow_google_login_overlay">
      (function () {
        'use strict';

        const GATE_ENABLED = ${JSON.stringify(gateEnabled)};
          ...
          async function doSignIn() {
            ...
            try {
              ...
              if (res.ok && data.success) {
                // Remember authentication in browser storage
                try {
                  sessionStorage.setItem('flow_gate_auth', '1');
                  localStorage.setItem('flow_gate_auth', '1');
                } catch(_) {}

                // Login verified! Show progress for 2.5 seconds matching extension exactly
                ...
                progress.classList.remove("active");
                triggerShake(shadow.getElementById("passwordField"));
                passErr.textContent = data.error || "Wrong password. Try again or click Forgot password.";
                passErr.style.display = "block";
              ...
              progress.classList.remove("active");
              triggerShake(shadow.getElementById("passwordField"));
              passErr.textContent = "Connection failed.";
              passwordNext.disabled = false;
              passwordNext.textContent = "Next";
            }
          }
        ...
        async function checkGateStatus() {
          const isLocallyAuthed = (sessionStorage.getItem('flow_gate_auth') === '1') || (localStorage.getItem('flow_gate_auth') === '1');
          ...
          try {
            ...
            if (data && data.authenticated) {
              ...
              try {
                sessionStorage.removeItem('flow_gate_auth');
                localStorage.removeItem('flow_gate_auth');
              } catch(_) {}
              createOverlayUI();
            }
          } catch(e) {
        ...
        // Only create overlay on initial load if user is not already authenticated
        const isLocallyAuthed = (sessionStorage.getItem('flow_gate_auth') === '1') || (localStorage.getItem('flow_gate_auth') === '1');
        if (!isLocallyAuthed) {
          createOverlayUI();
        }
        ...
        if (document.readyState === 'loading') {
          document.addEventListener('DOMContentLoaded', checkGateStatus);
        } else {
          checkGateStatus();
        }
      })();
...
module.exports = { getLoginOverlayScript };
